...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    '''on its own. Typically, you should only implement a function as
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    and the list is split in2 a "less than" sublist & a "greater than" sublist.
    ...
    if len(xs) <= 1:
        ...
        xs_great = [x for x in xs if cmp(x, pivot) == 1]
        xs_equal = [x for x in xs if cmp(x, pivot) == 0]
        xs_less = quick_sorted(xs_less, cmp=cmp)
        xs_great = quick_sorted(xs_great, cmp=cmp)
        return xs_less + xs_equal + xs_great
...
def quick_sort(xs, cmp=cmp_standard):
    ...
    Follow the pseudocode of the Lomuto partition scheme given'''
    '''on wikipedia
